class TicTacToe {
    constructor() {
        ...
        $("#gameModeSelection").on("change", ev => {
            ...
            //disables boxes
            $(".box").each(function () { if (!this.hasAttribute("disabled"))
            ...
            if (ev.target.value == "bot") {
                ...
                if (!document.getElementById("gameModus").classList.contains("invisible"))
                    document.getElementById("gameModus").classList.add("invisible");
                $(".box").each(function () { if (this.hasAttribute("disabled"))
        ...
        $(".box").on("click", (event) => {
            ...
            if (!this.availableFields.includes(id))
                return;
            ...
            if (this.current == 1 && this.opponent == "bot")
                document.getElementById("info").innerHTML = `It's the <b> bot's </b> turn!`;
            else if (this.current == 0 && this.opponent == "bot")
                document.getElementById("info").innerHTML = `It's <b> your </b> turn!`;
            else
                document.getElementById("info").innerHTML = `It's Player's <b> ${this.players[this.current]} </b> turn!`;
            const check = this.checkIfCompleted();
            if (check.winner) {
                if (this.opponent == "bot") {
                    if (check.winner == "X")
                        document.getElementById("info").innerHTML = `<b> You </b> won!`;
                    else
                        document.getElementById("info").innerHTML = `<b> Bot </b> wons!`;
                }
                else
                    document.getElementById("info").innerHTML = `Player <b> ${check.winner} </b> wons!`;
    ...
    makeBotMove(mode) {
        ...
        if (mode == "easy")
            botMoveMaker(this.availableFields[Math.floor(Math.random() * (this.availableFields.length - 1))]);
        else {
            const minimaxMove = this.bestMove();
            if (!minimaxMove)
                return botMoveMaker(this.availableFields[Math.floor(Math.random() * (this.availableFields.length - 1))]);
    ...
    */
    clearField(firstTimeYes) {
        $(".box").each(function () {
            this.textContent = null;
            if (this.hasAttribute("disabled"))
                this.removeAttribute("disabled");
            if (this.classList.contains("tictactoe-x"))
                this.classList.remove("tictactoe-x");
            else if (this.classList.contains("tictactoe-o"))
                this.classList.remove("tictactoe-o");
            else if (this.classList.contains("tictactoe-highlighted"))
                this.classList.remove("tictactoe-highlighted");
        });
        this.availableFields = ["A1", "A2", "A3", "B1", "B2", "B3", "C1", "C2", "C3"];
        this.board = [["", "", ""],
            ["", "", ""],
    ...
    checkIfCompleted() {
        ...
        for (let i = 0; i < fields.length; i++)
            if (!fields[i].textContent || fields[i].textContent == "")
                full = false;
        for (let i = 0; i < 3; i++) {
            // 3 senkrecht
            ...
            const checkWinner = () => {
                ...
                for (let i = 0; i < 3; i++) {
                    if (this.equals3(board[i][0], board[i][1], board[i][2]))
                        winner = board[i][0];
                    else if (this.equals3(board[0][i], board[1][i], board[2][i]))
                        winner = board[0][i];
                }
                ;
